...
export function HomeDashboard({ overview, chart }: Props) {
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <Card className="border-border/40 bg-card/50">
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-muted-foreground">Total Stablecoin Supply</span>
              <DollarSign className="h-4 w-4 text-muted-foreground/60" />
            </div>
            <p className="mt-3 text-2xl font-bold">{fmtUsd(overview.totalGlobalSupply)}</p>
              ...
              {overview.chains.map((chain, i) => {
                ...
                return (
                  <TableRow
                    ...
                    onClick={() => { if (cfg) window.location.href = `/chains/${cfg.slug}`; }}
                  >
      ...
      <Card className="border-border/40 bg-card/50 mb-8">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-medium text-muted-foreground">Top Stablecoins by Supply</CardTitle>
        </CardHeader>
        <CardContent className="px-0 pb-0">
          <Table>
            <TableHeader>
